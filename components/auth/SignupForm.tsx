...
export default function SignupForm() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    else { router.push("/profiles"); router.refresh(); }
  }
  ...
  const fields = [
    ...
    { label: "Password", type: showPass ? "text" : "password", value: password, onChange: setPassword, placeholder: "Min. 6 characters", isPassword: true },
    { label: "Confirm Password", type: showPass ? "text" : "password", value: confirmPassword, onChange: setConfirmPassword, placeholder: "Repeat password" },
  ];
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <div className="mb-10 text-center">
        ...
        <p className="mt-1 text-[13px]" style={{ color: "#555" }}>Start streaming in seconds</p>
      </div>

      <form onSubmit={handleSignup} className="space-y-3">
        {fields.map(({ label, type, value, onChange, placeholder, isPassword }) => (
          <div
            key={label}
          ...
          >
            ...
            <div className="flex items-center pr-3">
              ...
              />
              {isPassword && (
                <button type="button" onClick={() => setShowPass(!showPass)} style={{ color: "#444" }}>
      ...
      <div className="grid grid-cols-2 gap-2.5 mb-3">
        {([
          ...
          <motion.button
            ...
            className="flex items-center justify-center gap-2 rounded-xl py-2.5 text-[12px] font-medium text-zinc-400"
            style={{ background: "#0a0a0a", border: "1px solid #1f1f1f" }}
      ...
      <Link href="/browse">
        <motion.div className="w-full text-center rounded-xl py-2.5 text-[12px] font-medium" style={{ color: "#444" }} whileHover={{ color: "#888" }}>
          Continue as Guest →
        </motion.div>
      </Link>
