...
interface HistoryItem {
  id: string; profile_id: string; media_id: number; media_type: string;
  title: string; poster_path: string | null; watched_at: string;
}

interface HistoryClientProps { items: HistoryItem[]; profileId: string; }
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-4 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h2 className="text-xl font-semibold text-white mb-2 tracking-tight">No watch history</h2>
          <p className="text-[13px]" style={{ color: "#555" }}>Start watching to build your history.</p>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="px-4 pt-8 pb-16 md:px-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-semibold text-white tracking-tight" style={{ fontFamily: "var(--font-display, Syne, sans-serif)" }}>Watch History</h1>
